...
def _print_benchmark_table(
    ...
    cv_b0 = f"{bl.cv_recall_mean:.4f} +/- {bl.cv_recall_std:.4f}"
    cv_i3 = f"{best.cv_recall_mean:.4f} +/- {best.cv_recall_std:.4f}"
    cv_diff = f"{best.cv_recall_mean - bl.cv_recall_mean:+.4f}"
    ...
    print(
        f"  [+] CV Malignant Recall improved: {bl.cv_recall_mean:.4f} -> {best.cv_recall_mean:.4f} "
        f"(stability +/- {best.cv_recall_std:.4f})."
    )
