    ...
    <div className="space-y-gutter">
          ...
          <div className="text-center">
            {isLoading ? (
              ...
              <div className="inline-block bg-surface-container-low text-on-surface-variant px-6 py-2.5 rounded-full font-sans text-sm border border-outline-variant mb-2">
                Chờ nạp dữ liệu &amp; thực hiện chẩn đoán
              </div>
            ) : isMalignant ? (
              <div className="inline-block bg-error-container text-on-error-container px-6 py-2.5 rounded-full font-display-lg text-display-lg border border-error mb-2 font-bold">
            ...
            <p className="font-sans text-xs text-on-surface-variant flex items-center justify-center gap-1">
              ...
              Mức độ tin cậy của thuật toán: <strong>{hasResult ? `${confidence}%` : '--'}</strong>
            </p>
          </div>
      ...
      {decisionPath.length > 0 ? (
        ...
        <div className="bg-surface-container-low rounded-xl border border-outline-variant shadow-sm p-stack-md text-center text-xs font-sans text-on-surface-variant">
          Nhập các chỉ số hoặc chọn mẫu tham khảo phía trên và bấm <strong>Thực hiện chẩn đoán</strong> để theo dõi chi tiết từng bước phân nhánh của Cây Quyết định.
        </div>
      )}
    </div>
  );
