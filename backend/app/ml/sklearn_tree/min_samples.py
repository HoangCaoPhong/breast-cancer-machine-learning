...
class MinSamplesConfig:
    ...
    primary_metric: str = "malignant_f2"
    ...
    def __post_init__(self) -> None:
        ...
        if self.primary_metric != "malignant_f2":
            raise ValueError("primary_metric must be malignant_f2 under D-006")
    ...
    def from_mapping(cls, values: dict[str, Any]) -> MinSamplesConfig:
        ...
        return cls(
            ...
            primary_metric=str(values.get("primary_metric", "malignant_f2")),
        )
    ...
    def __post_init__(self) -> None:
        if not self.delta_vs_baseline:
            ...
            self.delta_vs_baseline = {
                ...
                "test_malignant_f2_delta": round(
                    b.test_metrics.malignant_f2 - bl.test_metrics.malignant_f2, 6
                ),
...
def run_min_samples_tuning(
    ...
    for candidate_order, (mss, msl) in enumerate(
        itertools.product(settings.min_samples_split_grid, settings.min_samples_leaf_grid)
    ):
    ...
    # D-006 selection rule: F2, recall, lower F2 std, simpler tree, candidate order.
    best_candidate = min(
        all_candidates,
        key=lambda r: (
            -round(r.cv_f2_mean, 12),
            -round(r.cv_recall_mean, 12),
            round(r.cv_f2_std, 12),
            r.n_leaves,
            r.fitted_depth,
            r.candidate_order,
        ),
...
def _evaluate_candidate(
    ...
    return GridCandidateResult(
        ...
        min_samples_split=mss,
        min_samples_leaf=msl,
        cv_f2_mean=float(np.mean(cv_f2s)),
        cv_f2_std=float(np.std(cv_f2s)),
