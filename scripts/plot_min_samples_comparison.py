...
def main() -> None:
    ...
    # ── 2. Grid Search CV F2 Heatmap (Top-Right) ──────────────────────────────
    ax2 = axs[0, 1]
    ...
    for c in result.all_candidates:
        ...
        heatmap_data[row_i, col_j] = c.cv_f2_mean

    im = ax2.imshow(heatmap_data, cmap="YlGnBu", aspect="auto", vmin=0.85, vmax=0.91)
    cbar = fig.colorbar(im, ax=ax2)
    cbar.set_label("5-Fold CV Malignant F2", fontsize=10)
    ...
    ax2.set_title("(B) Grid Search CV Malignant F2 Heatmap", fontsize=12, fontweight="bold")
